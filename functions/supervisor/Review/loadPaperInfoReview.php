...
$query=" select count(q.id) as total_questions , sum(q.value) as total_marks ,"
        . " (select score from scores where schedule_id=".$_POST['sid']." and user_id=".$_POST['uid']." ) as marks_scored from question q ,"
        . " schedule s where s.id = ".$_POST['sid']." and q.question_paper_id=s.question_paper_id";
//echo $query;
...
<div align='center'><button class='btn btn-warning' onClick="backFromExamReview()" >Go Back</button></div>
</div>
